...
class NeuralNetwork:
    ...
    def get_grid_dim(self, out_shape, stream=0):
        ...
        return (ceil(out_shape[0] / self.tpb[0]),
                ceil(out_shape[1] / self.tpb[1])), self.tpb, stream
    ...
    def backpropogation_cuda(self, inputs, targets, lr, batch,
        ...
        """
        # copy data to device
        inputs_d = cuda.to_device(inputs, stream=stream1) # 784 x batch
        targets_d = cuda.to_device(targets, stream=stream2) # 10 x batch
        inputs_d_T = cuda.to_device(inputs.T, stream=stream2)

        hidden_d = cuda.device_array((self.hidden_layer_sizes, batch), stream=stream1) # hidden_layer_sizes x batch

        # forward from IN layer to HIDDEN layer
        # IN --> HIDDEN
        cu_k.feedforward_step[self.get_grid_dim(hidden_d.shape, stream1)](inputs_d, self.weights[0], self.biases[0], hidden_d)
        # stream1.synchronize()

        # set output from HIDDEN layer to input for OUT layer
        inputs_d = hidden_d
        outputs_d = cuda.device_array((self.out_features, batch), stream=stream1)

        # forward from HIDDEN layer to OUT layer
        # HIDDEN --> OUT
        cu_k.feedforward_step[self.get_grid_dim(outputs_d.shape, stream1)](inputs_d, self.weights[1], self.biases[1], outputs_d)
        # stream1.synchronize()

        # create device array tensor
        # 10 x batch
        errors_d = cuda.device_array(targets.shape, stream=stream1)

        # create tensor for HIDDEN layer derivative
        hidden_errors_d = cuda.device_array((self.hidden_layer_sizes, batch), stream=stream2)
        
        # == > backward step / weights update step < ==
        
        # compute error
        cu_k.subtract[self.get_grid_dim(errors_d.shape, stream1)](targets_d, outputs_d, errors_d)
        stream1.synchronize()
        # stream2.synchronize()
        ...
        cu_k.matmul_T[self.get_grid_dim(hidden_errors_d.shape, stream2)](self.weights[1], errors_d, hidden_errors_d)
        stream2.synchronize()

        # gradient for HIDDEN
        gradient_d = cuda.device_array(outputs_d.shape, stream=stream1)
        # compute gradient for HIDDEN layer
        cu_k.gradient[self.get_grid_dim(gradient_d.shape, stream1)](outputs_d, errors_d, lr, gradient_d)


        # delta for HIDDEN layer bias
        delta_b_d = cuda.device_array(self.biases[1].shape, stream=stream1)
        # compute delta for HIDDEN layer bias by gradient
        cu_k.sum_cols[ceil(self.biases[1].shape[0] / 4), 4, stream1](gradient_d, delta_b_d)
        stream1.synchronize()

        # update HIDDEN layer bias by delta
        cu_k.add[self.get_grid_dim(self.biases[1].shape, stream1)](self.biases[1], delta_b_d)


        # delta for HIDDEN layer weights
        delta_w_d = cuda.device_array((gradient_d.shape[0], hidden_d.shape[0]), stream=stream2)
        # hidden_d.T
        # compute delta for HIDDEN layer weights by gradient
        cu_k.matmul_T[self.get_grid_dim(self.weights[1].shape, stream2)](gradient_d, hidden_d, delta_w_d)
        # cuda.synchronize()

        # update HIDDEN layer weights by delta
        cu_k.add[self.get_grid_dim(self.weights[1].shape, stream2)](self.weights[1], delta_w_d)

        # gradient for IN layer
        gradient_d = cuda.device_array(hidden_d.shape, stream=stream2)
        # compute gradient for IN layer
        cu_k.gradient[self.get_grid_dim(gradient_d.shape, stream2)](hidden_d, hidden_errors_d, lr, gradient_d)
        # cuda.synchronize()

        # delta for IN layer bias
        delta_b_d = cuda.device_array(self.biases[0].shape, stream=stream2)
        ...
        cu_k.add[self.get_grid_dim(self.biases[0].shape, stream2)](self.biases[0], delta_b_d)

        # delta for IN layer weights
        delta_w_d = cuda.device_array((gradient_d.shape[0], inputs.shape[0]), stream=stream1)
        # W * x.T

        # delta for IN layer weights
        cu_k.matmul[self.get_grid_dim(self.weights[0].shape, stream1)](gradient_d, inputs_d_T, delta_w_d)
        # cuda.synchronize()

        # update IN layer weights by delta
        cu_k.add[self.get_grid_dim(self.weights[0].shape, stream1)](self.weights[0], delta_w_d)
    ...
    def train(self, epochs: int, learning_rate: float, batch_size: int):
        ...
        with cuda.pinned(self.train_data), cuda.pinned(self.train_targets):
            for r in range(epochs):
                ...
                for i in range(batch_count):
                    ...
                    loss = np.mean(np.abs(loss)) # np.mean(loss ** 2)
                    epoch_loss += loss
                    print(f"\rEpoch: {r + 1} / {epochs} [{pr_bar}] {dur}s, loss: {loss:.5f}", end='')
                print()
                print(f'learning rate: {lr:.5f}')
                self.print_accuracy()
                print(f'mean epoch loss: {epoch_loss/batch_count:.5f}')
                print()
...
if __name__ == '__main__':
    ...
    dc = NeuralNetwork(784, 512, 10, mnist_dataset, act_f='sigmoid') # relu

    dc.train(20, 0.15, 100)
